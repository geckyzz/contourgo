...
func (s *SukebeiScraper) FetchTorrents(username string, keyword string, page int, sort string, order string) ([]NyaaTorrent, int, error) {
	var endpoint string
	if username != "" {
		endpoint = fmt.Sprintf("%s/sukebei/v1/user/%s/uploads?p=%d&s=%s&o=%s", s.proxyURL, url.PathEscape(username), page, url.QueryEscape(sort), url.QueryEscape(order))
	} else {
		endpoint = fmt.Sprintf("%s/sukebei/v1/?q=%s&p=%d&s=%s&o=%s", s.proxyURL, url.QueryEscape(keyword), page, url.QueryEscape(sort), url.QueryEscape(order))
	}
