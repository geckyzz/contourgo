...
func (b *DiscordBot) handleSlashTest(s *discordgo.Session, i *discordgo.InteractionCreate, optionMap map[string]*discordgo.ApplicationCommandInteractionDataOption) {
	...
	if service == "nyaa" || service == "sukebei" {
		...
		if inspect == "comments" {
			...
			err := b.AnnounceNyaaComment(i.ChannelID, service, torrentIDStr, targetTorrent.Name, firstComment, "", false, false)
			if err != nil {
		...
		if inspect == "comments" {
			...
			err := b.AnnounceATComment(i.ChannelID, service, firstComment.TorrentID, firstComment.Title, firstComment, "", false, false)
			if err != nil {
				b.sendFollowupMessage(s, i.Interaction, fmt.Sprintf("❌ Error creating test embed: %v", err))
			} else {
		...
		if inspect == "comments" {
			...
			firstComment := comments[0]
			err = b.AnnounceAnirenaComment(i.ChannelID, targetTorrent.ID, targetTorrent.FullTitle(), firstComment, "", targetTorrent.Uploader, false, false)
			if err != nil {
				b.sendFollowupMessage(s, i.Interaction, fmt.Sprintf("❌ Error creating test embed: %v", err))
			} else {
		...
		if inspect == "comments" || inspect == "result" {
			...
			err := b.AnnounceNekoBTComment(i.ChannelID, firstTorrent.Title, lastComment, "", false, false)
			if err != nil {
				b.sendFollowupMessage(s, i.Interaction, fmt.Sprintf("❌ Error creating test embed: %v", err))
			} else {
		...
		if inspect == "comments" {
			...
			firstComment := comments[0]
			err = b.AnnounceTsukihimeComment(i.ChannelID, firstTorrent.Name, firstComment, firstComment.ParentText, "", false, false)
			if err != nil {
				b.sendFollowupMessage(s, i.Interaction, fmt.Sprintf("❌ Error creating test embed: %v", err))
			} else {
